...
export default function() {
  console.log('🚀 9개 서버 로드 밸런싱 테스트 시작...');
  ...
  const apiOk = check(apiResponse, {
    ...
    'API 응답 시간': (r) => r.timings.duration < 5000,
  });
  ...
  const wsOk = check(wsResponse, {
    ...
    'WebSocket 응답 시간': (r) => r.timings.duration < 5000,
  });
  ...
  const chatOk = check(chatResponse, {
    ...
    '채팅방 응답 시간': (r) => r.timings.duration < 5000,
  });
  ...
  // 5. 제품 목록 테스트 (로드 밸런싱)
  const productsResponse = http.get(`${BASE_URL}/api/products?page=0&size=10`, {
    timeout: '10s'
  });
  
  const productsOk = check(productsResponse, {
    '제품 목록 접근 성공': (r) => r.status === 200 || r.status === 201,
    '제품 목록 응답 시간': (r) => r.timings.duration < 5000,
  });
  
  if (productsOk) {
    console.log('✅ 제품 목록 접근 성공 (로드 밸런싱)');
    successCount++;
    success = true;
  } else {
    console.log('❌ 제품 목록 접근 실패');
  }
  totalTests++;
