...
function performChatOverloadTest(testName, requestCount, messageCount) {
  console.log(`🔥 6개 서버 ${testName} 시작 - ${requestCount}번의 API 호출, ${messageCount}개의 메시지 전송`);
  ...
  for (let i = 0; i < requestCount; i++) {
    ...
    if (check(roomDetailResponse, {
      ...
      [`6개서버 ${testName} - 응답 시간 < 10초`]: (r) => r.timings.duration < 10000,
    })) {
      successCount++;
    } else {
      errorCount++;
    ...
    for (let j = 0; j < messageCount; j++) {
      const messageText = `6개서버-${testName} - 메시지 ${i + 1}-${j + 1} - ${new Date().toISOString()}`;
      const sendResponse = sendChatMessage(ROOM_ID, messageText);
      
      if (check(sendResponse, {
        [`6개서버 ${testName} - 메시지 전송 성공`]: (r) => r.status === 200 || r.status === 201,
      })) {
        messageSuccessCount++;
      } else {
        messageErrorCount++;
...
export default function() {
  ...
  group('6개 서버 Basic Chat Room Access', function () {
    ...
    if (check(response, {
      ...
      '6개서버 response time < 15s': (r) => r.timings.duration < 15000,
    })) {
      successCount++;
      success = true;
    }
  ...
  if (vuId <= 50) {
    ...
    group('6개 서버 Overload Test - 1000 Requests + 1000 Messages', function () {
    ...
    group('6개 서버 Overload Test - 1000 Requests + 5000 Messages', function () {
    ...
    group('6개 서버 Overload Test - 1000 Requests + 10000 Messages', function () {
    ...
    group('6개 서버 Overload Test - 1000 Requests + 30000 Messages', function () {
    ...
    group('6개 서버 Overload Test - 1000 Requests + 50000 Messages', function () {
