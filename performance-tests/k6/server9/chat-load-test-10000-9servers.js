...
function checkLoadBalancer() {
  ...
  console.log(`📈 9개 서버 상태: ${healthyServers}/${totalServers} (${(healthRatio * 100).toFixed(1)}%)`);
...
export default function() {
  ...
  group('9개 서버 Chat Rooms API Load Test', function () {
    totalTests++;
    response = http.get(`${BASE_URL}/api/chat/me/chatRooms?limit=25`);
    if (check(response, {
      '9개서버 chat rooms list accessible': (r) => r.status === 200 || r.status === 201,
    })) {
      successCount++;
      success = true;
    }
  ...
  // 2. 채팅방 상세 조회
  group('9개 서버 Chat Room Detail API', function () {
    totalTests++;
    response = http.get(`${BASE_URL}/api/chat/me/chatRooms/ROOM_1?includeMessages=true&limit=50`);
    if (check(response, {
      '9개서버 chat room detail accessible': (r) => r.status === 200 || r.status === 201,
    })) {
      successCount++;
      success = true;
    }
  ...
  // 3. 채팅방 메시지 조회
  group('9개 서버 Chat Messages API', function () {
    totalTests++;
    response = http.get(`${BASE_URL}/api/chat/me/chatRooms/ROOM_1/messages?limit=20`);
    if (check(response, {
      '9개서버 chat messages accessible': (r) => r.status === 200 || r.status === 201,
    })) {
      successCount++;
      success = true;
    }
  ...
  // 4. 안읽은 메시지 수 조회
  group('9개 서버 Unread Messages Count API', function () {
    totalTests++;
    response = http.get(`${BASE_URL}/api/chat/countALLUnreadMessages`);
    if (check(response, {
      '9개서버 unread messages count accessible': (r) => r.status === 200 || r.status === 201,
    })) {
      successCount++;
      success = true;
    }
