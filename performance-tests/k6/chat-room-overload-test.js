...
function checkLoadBalancingDistribution() {
  ...
  for (let i = 0; i < requestCount; i++) {
    ...
    if (response.status === 200 || response.status === 201) {
      // 성공한 요청을 모든 서버에 균등하게 분배 (실제로는 nginx가 처리)
...
function performChatRoomOverloadTest(testName, requestCount) {
  console.log(`🔥 ${testName} 시작 - ${requestCount}번의 API 호출`);
  ...
  for (let i = 0; i < requestCount; i++) {
    ...
    if (check(roomDetailResponse, {
      ...
      [`${testName} - 응답 시간 < 5초`]: (r) => r.timings.duration < 5000,
    })) {
      successCount++;
    } else {
      errorCount++;
...
export default function() {
  ...
  if (vuId <= 100) {
    // VU 1-100: 1000번 테스트
    group('Overload Test - 1000 Requests', function () {
    ...
    group('Overload Test - 100000 Requests', function () {
    ...
    group('Overload Test - 300000 Requests', function () {
