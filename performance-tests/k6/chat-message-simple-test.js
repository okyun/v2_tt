...
function performSimpleMessageTest(testName, messageCount) {
  console.log(`🔥 ${testName} 시작 - ${messageCount}개의 메시지 전송`);
  ...
  for (let i = 0; i < messageCount; i++) {
    const messageText = `${testName} - 메시지 ${i + 1} - ${new Date().toISOString()}`;
    ...
    if (messageCount >= 1000 && (i + 1) % 1000 === 0) {
      ...
      console.log(`📊 ${testName} 진행률: ${i + 1}/${messageCount} (${((i + 1) / messageCount * 100).toFixed(1)}%) - ${rate.toFixed(1)} msg/s`);
    }
...
export default function() {
  ...
  if (vuId <= 50) {
    // VU 1-50: 1000개 메시지
    group('Message Test - 1000 Messages', function () {
    ...
    // VU 51-100: 5000개 메시지
    group('Message Test - 5000 Messages', function () {
    ...
    group('Message Test - 10000 Messages', function () {
    ...
    group('Message Test - 30000 Messages', function () {
    ...
    group('Message Test - 50000 Messages', function () {
