...
function checkLoadBalancingDistribution() {
  ...
  const requestCount = 40; // 40번의 요청으로 분산 확인
...
function performSimpleMessageTest(testName, messageCount) {
  console.log(`🔥 6개 서버 ${testName} 시작 - ${messageCount}개의 메시지 전송`);
  ...
  for (let i = 0; i < messageCount; i++) {
    const messageText = `6개서버-${testName} - 메시지 ${i + 1} - ${new Date().toISOString()}`;
    ...
    if (messageCount >= 1000 && (i + 1) % 1000 === 0) {
      ...
      console.log(`📊 6개서버 ${testName} 진행률: ${i + 1}/${messageCount} (${((i + 1) / messageCount * 100).toFixed(1)}%) - ${rate.toFixed(1)} msg/s`);
    }
...
export default function() {
  ...
  group('6개 서버 Basic Chat Room Access', function () {
    ...
    if (check(response, {
      ...
      '6개서버 response time < 20s': (r) => r.timings.duration < 20000,
    })) {
      successCount++;
      success = true;
    }
  ...
  if (vuId <= 50) {
    // VU 1-50: 1000개 메시지
    group('6개 서버 Message Test - 1000 Messages', function () {
    ...
    // VU 51-100: 5000개 메시지
    group('6개 서버 Message Test - 5000 Messages', function () {
    ...
    group('6개 서버 Message Test - 10000 Messages', function () {
    ...
    group('6개 서버 Message Test - 30000 Messages', function () {
    ...
    group('6개 서버 Message Test - 50000 Messages', function () {
