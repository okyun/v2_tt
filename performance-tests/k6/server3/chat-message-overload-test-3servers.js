...
export const options = {
  stages: [
    { duration: '10s', target: 20 },    // 2분 동안 200명까지 증가
    { duration: '30s', target: 50 },    // 5분 동안 500명까지 증가
    { duration: '1m', target: 100 },  // 10분 동안 1000명까지 증가
...
const SERVERS = ['localhost:8081', 'localhost:8082', 'localhost:8083'];
...
function checkLoadBalancer() {
  ...
  SERVERS.forEach(server => {
    try {
      const response = http.get(`http://${server}/api/actuator/health`, {
        timeout: '10s'
      });
  ...
  console.log(`📈 3개 서버 상태: ${healthyServers}/${SERVER_COUNT} (${(healthRatio * 100).toFixed(1)}%)`);
  
  if (healthRatio >= 0.8) {
    console.log('✅ 3개 서버 로드 밸런서 상태 양호');
  } else if (healthRatio >= 0.5) {
    console.log('⚠️ 3개 서버 로드 밸런서 상태 주의');
  } else {
    console.log('❌ 3개 서버 로드 밸런서 상태 위험');
  }
...
function performChatOverloadTest(testName, requestCount, messageCount) {
  console.log(`🔥 3개 서버 ${testName} 시작 - ${requestCount}번의 API 호출, ${messageCount}개의 메시지 전송`);
  ...
  for (let i = 0; i < requestCount; i++) {
    ...
    if (check(roomDetailResponse, {
      [`3개서버 ${testName} - 채팅방 상세 조회 성공`]: (r) => r.status === 200 || r.status === 201,
      [`3개서버 ${testName} - 응답 시간 < 10초`]: (r) => r.timings.duration < 10000,
    })) {
      successCount++;
    } else {
      errorCount++;
    ...
    if (check(messagesResponse, {
      [`3개서버 ${testName} - 메시지 조회 성공`]: (r) => r.status === 200 || r.status === 201,
    })) {
      successCount++;
    } else {
      errorCount++;
    ...
    for (let j = 0; j < messageCount; j++) {
      const messageText = `3개서버-${testName} - 메시지 ${i + 1}-${j + 1} - ${new Date().toISOString()}`;
      const sendResponse = sendChatMessage(ROOM_ID, messageText);
      
      if (check(sendResponse, {
        [`3개서버 ${testName} - 메시지 전송 성공`]: (r) => r.status === 200 || r.status === 201,
      })) {
        messageSuccessCount++;
      } else {
        messageErrorCount++;
    ...
    if (check(unreadResponse, {
      [`3개서버 ${testName} - 안읽은 메시지 수 조회 성공`]: (r) => r.status === 200 || r.status === 201,
    })) {
      successCount++;
    } else {
      errorCount++;
    ...
    if (check(frontendResponse, {
      [`3개서버 ${testName} - 프론트엔드 페이지 접근 성공`]: (r) => r.status === 200,
    })) {
      successCount++;
    } else {
      errorCount++;
...
export default function() {
  ...
  group('3개 서버 Basic Chat Room Access', function () {
    ...
    if (check(response, {
      ...
      '3개서버 response time < 15s': (r) => r.timings.duration < 15000,
    })) {
      successCount++;
      success = true;
    }
  ...
  if (vuId <= 50) {
    ...
    group('3개 서버 Overload Test - 1000 Requests + 1000 Messages', function () {
    ...
    group('3개 서버 Overload Test - 1000 Requests + 5000 Messages', function () {
    ...
    group('3개 서버 Overload Test - 1000 Requests + 10000 Messages', function () {
    ...
    group('3개 서버 Overload Test - 1000 Requests + 30000 Messages', function () {
    ...
    group('3개 서버 Overload Test - 1000 Requests + 50000 Messages', function () {
