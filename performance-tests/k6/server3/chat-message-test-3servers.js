...
function performSimpleApiTest(testName, requestCount) {
  console.log(`🔥 3개 서버 ${testName} 시작 - ${requestCount}개의 API 요청`);
...
export default function() {
  ...
  // 1. 기본 API 접근 테스트
  group('3개 서버 Basic API Access', function () {
    totalTests++;
    const response = http.get(`${BASE_URL}/api/products?page=0&size=10`);
    if (check(response, {
      '3개서버 API access successful': (r) => r.status === 200 || r.status === 201,
  ...
  if (vuId <= 20) {
    // VU 1-20: 100개 요청
    group('3개 서버 API Test - 100 Requests', function () {
    ...
    // VU 21-40: 500개 요청
    group('3개 서버 API Test - 500 Requests', function () {
    ...
    // VU 41-60: 1000개 요청
    group('3개 서버 API Test - 1000 Requests', function () {
