...
export const options = {
  stages: [
    { duration: '30s', target: 10 },    // 1분 동안 100명까지 증가
    { duration: '50s', target: 30 },    // 3분 동안 300명까지 증가
    { duration: '2m', target: 100 },    // 5분 동안 500명까지 증가
...
function generateJWTToken() {
  ...
  if (response.status === 200) {
    ...
    console.log('🚫 JWT 토큰이 없어 테스트를 종료합니다.');
    throw new Error('JWT 토큰 생성 실패로 인한 테스트 종료');
  }
}
...
function checkLoadBalancingDistribution() {
  ...
  const requestCount = 20; // 20번의 요청으로 분산 확인
...
function performSimpleMessageTest(testName, messageCount) {
  console.log(`🔥 3개 서버 ${testName} 시작 - ${messageCount}개의 메시지 전송`);
  ...
  for (let i = 0; i < messageCount; i++) {
    const messageText = `3개서버-${testName} - 메시지 ${i + 1} - ${new Date().toISOString()}`;
    ...
    if (messageCount >= 1000 && (i + 1) % 1000 === 0) {
      ...
      console.log(`📊 3개서버 ${testName} 진행률: ${i + 1}/${messageCount} (${((i + 1) / messageCount * 100).toFixed(1)}%) - ${rate.toFixed(1)} msg/s`);
    }
...
export default function() {
  ...
  group('3개 서버 Basic Chat Room Access', function () {
    ...
    if (check(response, {
      ...
      '3개서버 response time < 20s': (r) => r.timings.duration < 20000,
    })) {
      successCount++;
      success = true;
    }
  ...
  if (vuId <= 50) {
    // VU 1-50: 1000개 메시지
    group('3개 서버 Message Test - 1000 Messages', function () {
    ...
    // VU 51-100: 5000개 메시지
    group('3개 서버 Message Test - 5000 Messages', function () {
    ...
    group('3개 서버 Message Test - 10000 Messages', function () {
    ...
    group('3개 서버 Message Test - 30000 Messages', function () {
    ...
    group('3개 서버 Message Test - 50000 Messages', function () {
