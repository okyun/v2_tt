#!/usr/bin/env node
/**
 * DAU 비트맵 vs Set — 「약 100명 동시 접속」 스테이지 시나리오로 각각 k6 실행 후 비교
 * (스크립트: dau-visit-100-users.js — 램프업 → 유지 → 램프다운)
 *
 * 사용
 ...
 *
 * 단일 JWT (모든 VU가 동일 사용자 → DAU 카운트는 1명 수준, 부하는 100VU 그대로)
 *   set JWT=...
 *   node compare-dau-bitmap-vs-set-100-users.js
 *
 * 환경변수
 *   BASE_URL              기본 http://127.0.0.1:8080
 *   JWT | TOKENS_FILE     하나 필수 (다양한 사용자 시뮬은 TOKENS_FILE 권장)
 *   PEAK_VUS              기본 100
 *   RAMP_DURATION         기본 30s
 *   HOLD_DURATION         기본 2m
 *   RAMP_DOWN_DURATION    기본 30s
 *   SLEEP_SEC             기본 0.15 (k6 반복 간 대기)
 *   K6_BIN                기본 k6
 *   KEEP_SUMMARY=1        .summary-dau-100u-*.json 유지
 *   REDIS_DOCKER_CONTAINER  선택 — 실행 후 BITCOUNT / SCARD
 */
...
const peakVus = process.env.PEAK_VUS || '100';
const rampDuration = process.env.RAMP_DURATION || '30s';
const holdDuration = process.env.HOLD_DURATION || '2m';
const rampDownDuration = process.env.RAMP_DOWN_DURATION || '30s';
const sleepSec = process.env.SLEEP_SEC || '0.15';

const summaryBitmap = path.join(dir, '.summary-dau-100u-bitmap.json');
const summarySet = path.join(dir, '.summary-dau-100u-set.json');
