...
function performChatRoomOverloadTest(testName, requestCount) {
  ...
  for (let i = 0; i < requestCount; i++) {
    ...
    if (check(roomDetailResponse, {
      ...
      [`3개서버 ${testName} - 응답 시간 < 5초`]: (r) => r.timings.duration < 5000,
    })) {
      successCount++;
    } else {
      errorCount++;
...
export default function() {
  ...
  group('3개 서버 Basic Chat Room Access', function () {
    ...
    if (check(response, {
      ...
      '3개서버 response time < 5s': (r) => r.timings.duration < 5000,
    })) {
      successCount++;
      success = true;
    }
  ...
  if (vuId <= 100) {
    ...
    group('3개 서버 Overload Test - 100000 Requests', function () {
    ...
    group('3개 서버 Overload Test - 300000 Requests', function () {
